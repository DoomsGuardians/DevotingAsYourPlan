...
namespace UnityEngine.UI
{
    ...
    public abstract class SmoothLayoutGroup : UIBehaviour, ILayoutElement, ILayoutGroup
    {
        ...
        /// <param name="axis">The axis to set the position and size along. 0 is horizontal and 1 is vertical.</param>
        /// <param name="pos">The position from the left side or top.</param>
        protected async void SetChildAlongAxisWithScale(RectTransform rect, int axis, float pos, float scaleFactor)
        {
            if (rect == null)
                return;
            ...
            await rect.DOAnchorPos(anchoredPosition, .35f).SetEase(Ease.OutQuad).AsyncWaitForCompletion();
        }
        ...
        /// <param name="pos">The position from the left side or top.</param>
        /// <param name="size">The size.</param>
        protected async void SetChildAlongAxisWithScale(RectTransform rect, int axis, float pos, float size, float scaleFactor)
        {
            if (rect == null)
                return;
