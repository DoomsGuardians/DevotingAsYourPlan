...
public class ChangeStatHistorialEffect : EventEffectSO
{
    ...
    [Tooltip("乘以变化值的倍数")] public float multiplier = 1f;

    [Tooltip("是否取反趋势（例如下降变为正值）")] public bool invertDelta = false;

    [Tooltip("无历史记录时跳过（否则使用 0 作为前值）")] public bool skipIfNoHistory = true;
    ...
    public override void Apply(EventInstance instance)
    {
        ...
        Debug.Log(
            $"[变化增减] {targetRole} 的 {statKey} 从 {previous} → {current}，Δ={delta:F2} * {multiplier} = {change:F2}");
    }
